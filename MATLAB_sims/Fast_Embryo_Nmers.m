...
%-------------------------------------------------------------------------
% DESCRIPTION
% simulate the CRISPR mutationswith  parameters from the experimental data
% the data is based on the "FAST" target in the embryo
% for the explanation see the notebook "Fast_sequence_Embryo_Reader.ipynb"
% the folder "../simulations" needs to be created to store the simulations and
% the frequencies for each mutational outcome (to be used later)
%-------------------------------------------------------------------------
...
% prob for each of Nmers (53 in total)
...
for r = 1:Nsims                      % TO HAVE MULTIPLE SAMPLES
    ...
    for n_div = 1:Ndiv                           %for each division
        ...
        % the site can mutate only once, into one of 60 possible states
        Aind = find(A2 == 0);
        AR = rand(size(Aind));
